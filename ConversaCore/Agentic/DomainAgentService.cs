...
namespace ConversaCore.Agentic; 

/// <summary>
/// Abstract base class for domain-specific agent services.
...
public abstract class DomainAgentService {
    ...
    protected readonly IConversationContext _context;
    protected readonly TopicWorkflowContext _wfContext;
    protected readonly ILogger<DomainAgentService> _logger;
    ...
    protected string? _activeCardId;
    protected const string ActiveCardKey = "ActiveCardId";
    protected readonly Stack<TopicFlow.TopicFlow> _pausedTopics = new Stack<TopicFlow.TopicFlow>(); // Track paused topics
    protected readonly Dictionary<string, PendingSubTopic> _pendingSubTopics = new();
    ...
    #region Abstract Event Handlers - MUST be implemented by domain services
    ...
    /// Domain implementations can override for custom message processing.
    /// </summary>
    protected virtual async Task OnUserMessageReceivedAsync(string message, CancellationToken ct)
    {
    ...
    /// Domain implementations can override for custom card processing.
    /// </summary>
    protected virtual async Task OnCardSubmittedAsync(Dictionary<string, object> data, CancellationToken ct)
    {
    ...
    /// Domain implementations can override for custom reset logic.
    /// </summary>
    protected virtual async Task OnConversationResetRequestedAsync(CancellationToken ct)
    {
    ...
    /// <summary>
    /// Processes a user message by finding and executing the best matching topic.
    /// PROTECTED - only callable internally via event handlers.
    /// </summary>
    protected async Task ProcessUserMessageAsync(string userMessage, CancellationToken ct = default) {
        ...
        var fallbackTopic = _topicRegistry.GetTopic("FallbackTopic") as TopicFlow.TopicFlow;
        ...
        if (topic == null) {
            ...
            if (_activeTopic is TopicFlow.TopicFlow activeFlow &&
        ...
        if (topic is not TopicFlow.TopicFlow flow) {
            LogWarn("0010004", topic.GetType().Name);
            return;
        }

        if (_activeTopic is TopicFlow.TopicFlow currentActiveTopic)
    ...
    /// <summary>
    /// Starts the conversation by initializing the conversation start topic.
    /// PROTECTED VIRTUAL - domain services should override or call via event handler.
    /// </summary>
    protected virtual Task StartConversationAsync(CancellationToken ct = default)
        => throw new NotImplementedException(
            $"{GetType().Name} must override StartConversationAsync");
    ...
    protected async Task ResetConversationAsync(CancellationToken ct = default) {
        ...
        if (_activeTopic is TopicFlow.TopicFlow currentTopic) {
        ...
        foreach (var topic in _topicRegistry.GetAllTopics()) {
            if (topic is TopicFlow.TopicFlow tf) {
                LogInfo("0010013", tf.Name);
                tf.Reset();
            }
        }

        if (_topicRegistry.GetTopic("ConversationStart") is TopicFlow.TopicFlow conversationStartTopic) {
            LogInfo("0010014");
            ...
            if (_topicRegistry.GetTopic("ComplianceTopic") is TopicFlow.TopicFlow complianceTopic) {
    ...
    protected class PendingSubTopic {
        public TopicFlow.TopicFlow CallingTopic { get; set; } = null!;
        public TopicFlow.TopicFlow SubTopic { get; set; } = null!;
    ...
    protected async void OnTopicTriggered(object? sender, TopicTriggeredEventArgs e) {
        ...
        var senderId = sender is TopicFlowActivity activity
            ? activity.Id
            : (sender as TopicFlow.TopicFlow)?.Name ?? "Unknown";

        LogInfo("EVT_TT_0001"); // TopicTriggered received

        // Console debug left intact
        Console.WriteLine($"[DomainAgentService.OnTopicTriggered] *** EVENT RECEIVED ***");
        ...
        Console.WriteLine($"[DomainAgentService.OnTopicTriggered] *** SENDER DETAILS *** Sender type: {sender?.GetType().FullName}, Is TopicFlow: {sender is TopicFlow.TopicFlow}, Is Activity: {sender is TopicFlowActivity}");
        ...
        if (nextTopic is TopicFlow.TopicFlow nextFlow) {
            ...
            if (waitForCompletion) {
                ...
                if (_activeTopic is TopicFlow.TopicFlow currentFlow) {
    ...
    protected async void OnTopicLifecycleChanged(object? sender, TopicLifecycleEventArgs e) {
        ...
        try {
            ...
            if (e.State == TopicLifecycleState.Completed && e.TopicName == "FallbackTopic") {
                ...
                var fallback = _topicRegistry.GetTopic("FallbackTopic") as TopicFlow.TopicFlow;
                if (fallback == null) {
                    LogWarn("EVT_LF_0005");
                    return;
                }
    ...
    protected void OnCardStateChanged(string cardId, CardState newState) {
        LogInfo("EVT_CS_0001");
        CardStateChanged?.Invoke(this, new CardStateChangedEventArgs(cardId, newState));
    }

    protected void OnMessageReady(string content) {
    ...
    /// <summary>
    /// Handles adaptive card submission.
    /// PROTECTED - only callable internally via event handlers.
    /// </summary>
    ...
    protected async Task HandleFallbackCompletion(TopicFlow.TopicFlow fallbackTopic, TopicResult result) {
        if (_pausedTopics.Count == 0) {
            LogInfo("EVT_FB_0001");
            return;
        }
    ...
    protected void HandleAsyncActivityCompleted(object? sender, AsyncQueryCompletedEventArgs e) {
        ...
        if (_activeTopic is not TopicFlow.TopicFlow flow) {
            LogWarn("EVT_AS_0003");
            AsyncActivityCompleted?.Invoke(this, e);
            return;
        }
    ...
    protected async Task HandleSubTopicCompletion(TopicFlow.TopicFlow completedSubTopic, TopicResult subTopicResult) {
    ...
    protected void HandleCardJsonEmitted(object? s, EventArgs e) => LogTrace("EVT_DBG_0001");
    protected void HandleCardJsonSending(object? s, EventArgs e) => LogTrace("EVT_DBG_0002");
    protected void HandleCardJsonRendered(object? s, EventArgs e) => LogTrace("EVT_DBG_0003");
    protected void HandleCardDataReceived(object? s, EventArgs e) => LogTrace("EVT_DBG_0004");
    protected void HandleModelBound(object? s, EventArgs e) => LogTrace("EVT_DBG_0005");

    #endregion

    #region Event Wiring
    protected void HookTopicEvents(TopicFlow.TopicFlow flow) {
        // ===== TOPIC-LEVEL EVENTS =====
        ...
        if (flow is ITopicTriggeredActivity topicTrigger) {
            Console.WriteLine($"[DomainAgentService.HookTopicEvents] Subscribing to TopicTriggered for topic '{flow.Name}'");
        ...
        _logger.LogInformation("[DomainAgentService] ✔ Hooked topic '{Topic}' and all activities", flow.Name);
    }
    protected void UnhookTopicEvents(TopicFlow.TopicFlow flow) {
        if (flow == null)
            return;

        _logger.LogInformation("[DomainAgentService] Unhooking events for topic flow: {FlowName}", flow.Name);

        // --- 🔹 Deactivate any card still active ---
        var activeCard = _context.GetValue<string>(ActiveCardKey);
        if (!string.IsNullOrEmpty(activeCard)) {
            _logger.LogInformation("[DomainAgentService] Deactivating lingering active card {CardId} before unhook", activeCard);
        ...
        _logger.LogInformation("[DomainAgentService] ✅ Unhooked topic and activity events for {FlowName}", flow.Name);
    }
    protected void HookActivityEvents(TopicFlowActivity child) {
        ...
        _logger.LogInformation(
            "[DomainAgentService] ✔ Hooked runtime activity '{Id}' ({Type})",
            child.Id, child.GetType().Name);
    }
    protected void UnhookActivityEvents(TopicFlowActivity child) {
        // --- Lifecycle ---
        child.ActivityLifecycleChanged -= HandleActivityLifecycleChanged;
        child.MessageEmitted -= HandleMessageEmitted;
    ...
    protected string ResolveLogMessage(string codeOrMessage, params object[] args) {
        if (_logMessages.TryGetValue(codeOrMessage, out var template))
            return string.Format(template, args);

        return string.Format(codeOrMessage, args);
    }
    protected void LogInfo(string codeOrMessage, params object[] args)
        => _logger.LogInformation(ResolveLogMessage(codeOrMessage, args));
    protected void LogWarn(string codeOrMessage, params object[] args)
        => _logger.LogWarning(ResolveLogMessage(codeOrMessage, args));
    protected void LogError(string codeOrMessage, params object[] args)
        => _logger.LogError(ResolveLogMessage(codeOrMessage, args));
    protected void LogTrace(string codeOrMessage, params object[] args) {
#if DEBUG
        _logger.LogDebug(ResolveLogMessage(codeOrMessage, args));
#endif
    }

    protected readonly Dictionary<string, string> _logMessages = new()
{
...
{ "ACR_0003", "[AskCaliforniaResidency] Card created successfully" },
