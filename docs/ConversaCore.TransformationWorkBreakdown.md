...
- [x] **CC-203 — Implement `ITopicActivator`.** Resolve a fresh mutable topic execution from the current conversation scope and await initialization before exposing it. Defines the optional `IAsyncInitializable` seam now used by InsuranceAgent's T1 topic. See [#33](https://github.com/lg061870/InsuranceSemanticV2/issues/33).
...
- [x] **CC-209 — Eliminate constructor background initialization.** `IAsyncInitializable` and `TopicActivator` provide and test the awaited topic build/activation phase; ConversaCore contains no topic constructor with untracked initialization, and InsuranceAgent's T1 topic now consumes the seam.
...
- [x] **CC-501 — Move startup/compliance composition into topics.** The registered `InsuranceConversationStartTopic` owns startup and compliance composition, and end-to-end tests cover every TCPA/California consent branch without service-inserted activities.
- [x] **CC-502 — Replace the domain agent injection.** `Home.razor` injects the scoped `IConversationRuntime`; the active application path no longer registers or calls `InsuranceAgentServiceV2` or `SubscribeToChatWindowEvents`, and two-circuit tests prove scoped isolation.
- [x] **CC-503 — Define typed insurance host contracts.** Immutable progress, customer-console, navigation, and qualification contracts cross the host boundary; `Home.OnHostOutput` consumes each typed notification without workflow-context access.
- [x] **CC-504 — Move lead creation into a tool.** `CreateLeadTool` returns the created ID into conversation state, `Home.razor` owns no `currentLeadId`, and end-to-end tests cover successful creation, failure containment, and cross-circuit ID isolation.
- [x] **CC-505 — Move profile persistence into tools.** Contact, life goals, coverage intent, health, dependents, employment, and beneficiary writes execute as topic-owned tools, with end-to-end success, failure-containment, reset, and isolation coverage.
- [x] **CC-506 — Migrate dialogs and questions.** The active insurance flow uses standard adaptive-card activities, consent retry resets the scoped runtime directly, and framework/insurance tests cover card continuation, correlated host interaction, reset, fallback interruption, and resumption.
- [x] **CC-507 — Preserve visual reactions.** `MarketingT1Topic` publishes typed qualification-progress, completion, and customer-console notifications; the host consumes them through `OnHostOutput`, independently of durable tool effects.
- [x] **CC-508 — Migrate Zapier usage.** `ZapierIntegrationDemoTopic` invokes the registered `zapier.webhook` tool and derives confirmation from its typed result; focused transport tests cover configured endpoint delivery, safe HTTP-failure mapping, and caller cancellation.
...
- [x] **CC-601 — Add a minimal bounded generated-style topic.** Migrated the maintained and staged Blazor templates (`SampleTopic`) to `ComposedTopicFlow`, explicit constructor injection (`TopicWorkflowContext`, `ILogger<SampleTopic>`, `IWorkflowActivityFactory`), immutable `PromptActivityDefinition`, `QuickAnswerActivityDefinition`, and `GeneratedAdaptiveCardDefinition` with typed model `SampleInputModel`. Demonstrated affirmative card flow, bounded fallback branch (`FallbackActivity`), clean completion, and safe recomposition on reset without constructor rebuild. Verified with dedicated automated tests in `SampleTopicGeneratedAuthoringTests`.
- [x] **CC-602 — Add a tool sample.** Demonstrated one read-only tool (`SampleLookupTool`, `sample.item.lookup`) and one confirmed mutating tool (`SampleOrderTool`, `sample.order.create`) with strongly typed contracts and DataAnnotations. Demonstrated bounded topic authoring using `InvokeToolActivity<TTool, TRequest, TResult>` and `ComposedTopicFlow` (`SampleToolTopic`). Enforced strict topic allowlists (`AllowedToolIds`) restricting tool execution, and proved that confirmation for mutating tools is derived strictly from explicit user selection (QuickAnswer button click), never from LLM/model output. Maintained 1-to-1 parity across `ConversaCore.SDK/ConversaCore.BlazorTemplateHost/` and `templates/conversacore-blazor/`, added `AI_DEVELOPER_GUIDE.md`, and verified full lifecycle, allowlist rejection, and confirmation behavior with 7 dedicated automated tests in `SampleToolGeneratedAuthoringTests`.
- [x] **CC-603 — Add typed host-output samples.** Added reusable `PublishHostNotificationActivity<TPayload>` and `InvokeHostInteractionActivity<TRequest, TResponse>` activities to `ConversaCore.TopicFlow.Activities`. Demonstrated typed one-way notifications (`SampleHostNotification`, `sample.host.notification`) and correlated two-way host interactions (`SampleHostInteractionRequest` / `SampleHostInteractionResponse`, `sample.host.decision`) in `SampleHostOutputTopic` (`sample.host.start`) without anonymous `EventTriggerActivity` payloads or leaking workflow context to the host. Updated `Index.razor` host UI to handle `OnHostOutput`, render host notifications, and provide interactive response buttons calling `context.RespondAsync(...)`. Maintained 1-to-1 parity across `ConversaCore.SDK/ConversaCore.BlazorTemplateHost/` and `templates/conversacore-blazor/`, updated `AI_DEVELOPER_GUIDE.md`, and added automated integration tests in `SampleHostOutputGeneratedAuthoringTests`.
- [x] **CC-604 — Update the topic authoring guide.** Replaced the outdated draft in `docs/ConversaCore.TopicAuthoringGuide.md` with the comprehensive, canonical topic authoring guide. Fully documented the 4-layer architecture, topic descriptors and DI registration, the `ComposedTopicFlow` post-construction lifecycle, the explicit `IAsyncInitializable` exception for dynamic rule loading, state and context keys, subtopic hand-down/regain coordination, generated adaptive cards (`GeneratedAdaptiveCardDefinition` with typed models and DataAnnotations) versus rich domain cards, typed tools and allowlist enforcement, mutation safety and human confirmation policies, typed host outputs (`PublishHostNotificationActivity` and `InvokeHostInteractionActivity`) and Blazor host UI integration (`OnHostOutput`), idempotent reset and cancellation lifecycles, and strict negative rules (prohibited patterns).
- [x] **CC-605 — Publish the generator integration contract.** Published the canonical integration contract in `docs/ConversaCore.GeneratorIntegrationContract.md` defining the supported JSON diagram → generated C# → Roslyn compilation → scoped activation pipeline for visual diagram code generators (ScriptEditor#46) and AI assistants. Formally defined the ScriptEditor#46 ownership boundary (diagram schema, Roslyn code emission, assembly build) versus ConversaCore (public framework contracts, runtime execution, typed outputs, reset lifecycle); published the Activity Constructor Capability Matrix mapping diagram nodes to literal-safe activities, explicit scoped authoring factories (`IWorkflowActivityFactory`), compile-time generic activities (`InvokeToolActivity`, `PublishHostNotificationActivity`, `InvokeHostInteractionActivity`), and subtopic orchestration; documented explicit DI constructor capture and `IConversaCoreBuilder` registration emission; documented bounded definitions (`GeneratedAdaptiveCardDefinition` validation constraints, allowlisted input kinds) and typed DataAnnotations form models; and provided a complete compilable reference generated topic flow. Cross-referenced in `ConversaCore.TopicAuthoringGuide.md` and `ConversaCore.SDK/README.md`.
- [x] **CC-606 — Add template and generated-consumer validation tests.** Fixed template exclude patterns in `templates/conversacore-blazor/.template.config/template.json` and `ConversaCore.SDK/ConversaCore.BlazorTemplateHost/templates/conversacore-blazor/.template.config/template.json` to exclude `**/*.ps1` instead of `tools/**`, ensuring domain tools in `Tools/` (`SampleLookupTool`, `SampleOrderTool`) are preserved on Windows while excluding build scripts. Added `TemplateGeneratedConsumerValidationTests` to `ConversaCore.Tests/Authoring/`: verified `dotnet new conversacore-blazor` instantiates a fresh standalone project in a temporary directory, restores, and builds with 0 errors producing runnable binaries without repository project references; verified two concurrent conversation scopes executing the template's primary generated flow (`SampleTopic`) maintain strict isolation of conversation IDs, topic instances, activity instances, card submissions, typed `SampleInputModel` context state, and presentation view states; and proved that resetting one scope cancels and recomposes its own topic graph without terminating, mutating, or clearing the second scope.
- [x] **CC-607 — Validate package consumption and compiler compatibility.** Added official NuGet package metadata (ID, version 1.0.0, authors, descriptions, tags) to `ConversaCore.csproj` and `ConversaCore.UI.csproj`, and packed both into `artifacts/packages`. Updated the maintained template (`ConversaCore.BlazorTemplateHost.csproj` and `template.json`) to support package consumption mode (`--usePackages true` / `ConversaCoreUsePackages=true`). Added `PackageConsumptionValidationTests` to `ConversaCore.Tests/Authoring/`: verified package metadata and dependencies (`ConversaCore.UI` depends on `ConversaCore` 1.0.0 and bundles static assets); verified a fresh standalone project consuming only the packed NuGet packages (zero project references, zero copied `lib/` DLLs) restores, builds with 0 errors, and runs full single-scope lifecycle, two-circuit concurrency isolation, and independent reset isolation; and verified that an instantiated template project with its entire `lib/` directory deleted restores and builds cleanly with 0 errors using the packages. Recorded package versions (1.0.0) and confirmed that ScriptEditor#46's external compile evidence will link to this package target. This completes all 7 tasks in WP6.
...
- [x] **CC-700 — Mark legacy APIs obsolete.** Added actionable migration messages and docstrings for `DomainAgentService`, `ConfigureTopics`, `ResetConversaCore`, legacy custom events (`ICustomEventTriggeredActivity`, `CustomEventTriggeredEventArgs`, `EventTriggerActivity`, `ChatPromptAttentionActivity`, `ShowSuggestionsActivity`, `TopicFlow.CustomEventTriggered`, `ChatInteractionPayload`), provider-specific workflow activities (`WhatsAppMessageActivity`), and obsolete orchestration/builders (`FlowBuilder`, `TopicRegistry`, `TopicManager`, `AddTopicsFromLegacyRegistrations`, and `CustomChatWindowV3.AgentService`). Directs users toward `IConversationRuntime`, `TopicDescriptor`, typed tools, and `ComposedTopicFlow` without invalidating justified direct async initialization (`IAsyncInitializable`). Residual consumers inventoried for progressive retirement in CC-701 through CC-705.
- [x] **CC-701 — Remove unused agent implementations.** Deleted `InsuranceAgentService` and `InsuranceAgentServiceV2`, along with unreferenced experimental bridges `HybridChatService`, `HybridChatServiceV2`, and dead controller `ConversationController.cs`. Cleaned up residual docstrings and unused injection comments in `Home.razor` and `TopicResult.cs`. Verified zero production call sites, 0 compiler errors across the solution, 63 characterization/authoring tests passing, and 13 end-to-end and two-circuit tests passing in `InsuranceSemanticV2.IntegrationTests`.
- [x] **CC-702 — Retire duplicate routing/runtime paths.** Removed obsolete `TopicRegistry.ConfigureTopics` startup scope configuration from `InsuranceAgent/Program.cs`. Deleted unintegrated parallel runtime `ConversaCore/NEWCODE/*` (`TopicFlowV2.cs`, `TopicFlowActivityV2.cs`) and obsolete `ExecuteTopicActivity.cs`. Confirmed host operates exclusively via single `IConversationRuntime`, `ITopicCatalog`, `ITopicRouter`, and `IWorkflowRunner` routing authority. All 63 characterization and authoring tests pass with 0 errors.
- [x] **CC-703 — Remove unsafe lifecycle and authoring patterns.** Eliminated reflection-based state resets across `DomainAgentService`, `ConversationStartTopic`, and `TopicWorkflowContextExtensions` by introducing explicit framework-owned `TopicFlow.ForceState`, `TopicFlow.GetActivity`, and `TopicWorkflowContext.TryGetValue`. Verified zero constructor `Task.Run` calls, zero `async void` handlers, and proper use of `ComposedTopicFlow` with justified direct `IAsyncInitializable` retained for async rule loading. All 63 characterization and authoring tests pass with 0 errors.
- [x] **CC-704 — Consolidate DI registration.** Consolidated DI registration across framework and host by removing duplicate `AddOpenAIEmbeddingGenerator` and duplicate singleton `IVectorDatabaseService` from `InsuranceAgent/Program.cs`. Verified single scoped `IWorkflowActivityFactory` and `TopicWorkflowContext` registration in framework foundation (`ConversaCoreBuilderRoutingExtensions`), with no capture by singletons. Verified with 0 errors across the solution and 13 passing integration tests.
- [x] **CC-705 — Resolve demo and topic leftovers.** Removed incomplete and machine-specific demo topics `GenericDecisionDemoTopic.cs` and `EventTriggerDemoTopic.cs` (superseded by framework-owned `ConversationRuntimeTests.EventTriggerDemoTopic_UsesNonBlockingNotificationsAndCorrelatedInteraction`), cleanly deregistered them from `AddInsuranceTopics.cs` and `Program.cs`, and removed the unused `ITopicTriggeredActivity` interface and unused `TopicTriggered` event from `ComplianceTopic.cs` while preserving justified rich domain cards (`ComplianceCard`, `LeadDetailsCard`, `BeneficiaryCard`, `CoverageIntentCard`, `ContactInfoCard`, `HealthInfoCard`, `DependentsCard`, `EmploymentCard`, `LifeGoalsCard`). Full solution builds with 0 errors and all 63 characterization/authoring and 9 runtime tests pass.
- [x] **CC-706 — Supersede obsolete documentation.** Marked `docs/V3_EVENT_DRIVEN_REFACTORING_GUIDE.md` and `REFACTORING_PROGRESS.md` as historical and superseded with prominent notices pointing to canonical docs (`ConversaCore.TopicAuthoringGuide.md`, `ConversaCore.TargetArchitecture.md`, `ConversaCore.GeneratedAuthoringArchitecture.md`, `ConversaCore.GeneratorIntegrationContract.md`, and ADR-007). Updated `INTEGRATION_QUICKSTART.md` to reference the supported host-output topic pattern (`SampleHostOutputTopic.cs`) instead of deleted demo topics.
- [x] **CC-707 — Regenerate architecture artifacts.** Regenerated target architecture artifacts using Archify (v2.16): updated `conversacore-current-product-architecture.json` and delivered `.html` showing `IConversationRuntime` single facade, immutable `ITopicCatalog`, `ComposedTopicFlow`, and typed tools; updated `conversacore-current-topic-execution-sequence.json` and delivered `.html` showing runtime coordination, active topic execution, and typed output stream dispatch; created `conversacore-generated-authoring-pipeline.json` and delivered `.html` documenting the ScriptEditor#46 C# generator boundary, scoped DI activation, `ComposedTopicFlow` post-DI composition, and allowlisted tool policy gating with 0 runtime JSON interpreter. All diagrams pass Archify showcase validation with 0 errors and 0 warnings.
- [x] **CC-708 — Run dependency, copied-binary, and dead-code review.** Removed `CopyToBlazorTemplateHostLib` post-build copy targets from `ConversaCore.csproj` and `ConversaCore.UI.csproj`, permanently halting git worktree dirtying. Deleted all 8 repository-copied binary DLLs and obsolete `README.txt` files from `lib/` folders across `ConversaCore.SDK/ConversaCore.BlazorTemplateHost/lib/`, `templates/conversacore-blazor/lib/`, `InsuranceLeadsAgent/lib/`, and repository root `lib/`. Updated template configurations (`template.json`) to exclude `lib/**` and default to `usePackages=true`. Migrated template project files to pure package consumption (`<PackageReference Include="ConversaCore" Version="1.0.0" />` and `ConversaCore.UI`), eliminating all DLL `HintPath` fallbacks. Updated `TemplateGeneratedConsumerValidationTests` to verify package-based builds and assert zero `lib` folder exists in instantiated projects. Full solution builds with 0 errors and zero binary churn. This completes all 9 tasks in WP7.
