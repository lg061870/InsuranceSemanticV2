...
- [ ] **CC-204 — Consolidate routing into `ITopicRouter`.** Merge the useful behavior of `TopicRegistry` and `TopicManager` under one policy and one threshold model. Implementation in progress: catalog-only selection, explicit first-refusal decisions, deterministic trigger phrases, optional bounded semantic ranking, and explicit system fallback. See [routing contract and integration boundaries](ConversaCore.TopicRouting.md); runner consumption remains CC-205/207. No InsuranceAgent migration in this delivery.
- [ ] **CC-205 — Implement `IWorkflowRunner`.** Own activity execution, active cursor, waits, output dispatch, and terminal transitions. Implementation in progress: scoped retention of one activated topic, serialized command execution, immutable outcome dispatch, and terminal/wait transitions; see [workflow-runner boundaries](ConversaCore.WorkflowRunner.md). Subtopic coordination, interruption, reset, typed public output, and runtime-facade assembly remain separately tracked.
- [ ] **CC-206 — Move subtopic coordination into the runner.** Replace event-subscription choreography with an explicit call stack and awaitable child completion. Implementation in progress: runner-owned parent retention, catalog-validated child activation, session-stack ownership tracking, and deterministic parent resumption; see [subtopic coordination](ConversaCore.SubtopicCoordination.md). Typed continuation data and reset/cancellation interaction remain separate tasks.
- [ ] **CC-207 — Implement interruption and fallback policy.** Give waiting topics first refusal, permit declared interruptions, and resume the original topic deterministically. Implementation in progress: `IConversationMessageCoordinator` composes router and runner for real first-refusal delivery, declined-input rerouting, declared interruption, explicit fallback, and saved-instance resumption; see [interruption policy](ConversaCore.InterruptionPolicy.md). Reset/cancellation and facade/output assembly remain separate.
- [ ] **CC-208 — Implement reset and cancellation.** Remove reflection-based state forcing and propagate cancellation through all active operations. Implementation in progress: runner-owned linked cancellation generation plus public cancel/reset operations that drain the serialized command, clear retained executions, and reset the session without reflection; see [reset and cancellation](ConversaCore.ResetAndCancellation.md). Start-topic restart and external-operation cancellation remain separate integration work.
...
- [ ] **CC-210 — Replace internal `async void` paths.** Convert event handlers and callbacks to task-returning runtime operations with centralized exception handling. Framework implementation complete pending test gates: semantic and integration operations are awaited, Repeat/Composite continuations are tracked with owner-level failure handling, legacy event shims observe task-returning callbacks centrally, and executable scans contain no `async void`, discarded async calls, or `Task.Run`.
