...
{
    // Tobacco Use Selection (single choice)
    [JsonPropertyName("tobacco_use")]
    public string? TobaccoUse { get; set; }

    // Medical Condition Selections (multiple choice with "None Apply" exclusivity)
    ...
    // Health Insurance Selection (single choice)
    ...
    public bool? UsesTobacco
    {
        get
        {
            return TobaccoUse?.ToLower() switch
            {
                "yes" => true,
                "no" => false,
                _ => null
    ...
    public bool? HasHealthInsurance
    {
        get
        {
            return HealthInsurance?.ToLower() switch
            {
                "yes" => true,
                "no" => false,
                _ => null
    ...
    // Validation for medical conditions mutual exclusivity
    public bool HasValidMedicalConditionSelection
    {
        get
        {
            var hasNoneApply = ConditionNone?.ToLower() == "true";
            var hasOtherConditions = ConditionDiabetes?.ToLower() == "true" || 
                                   ConditionHeart?.ToLower() == "true" || 
                                   ConditionBloodPressure?.ToLower() == "true";
            
            // Either "None Apply" is selected OR other conditions are selected, but not both
            return hasNoneApply != hasOtherConditions;
        }
    }

    // Method to clean up conflicting medical condition selections
    public void NormalizeMedicalConditions()
    {
        // If "None Apply" is selected, clear other conditions
        if (ConditionNone?.ToLower() == "true")
        {
            ConditionDiabetes = "false";
            ConditionHeart = "false";
            ConditionBloodPressure = "false";
        }
        // If any other condition is selected, clear "None Apply"
        else if (ConditionDiabetes?.ToLower() == "true" || 
                 ConditionHeart?.ToLower() == "true" || 
                 ConditionBloodPressure?.ToLower() == "true")
        {
            ConditionNone = "false";
